///////////////////////////////////////////////////////////////////////////////
//
//  Island Coalescence Problem - Fadeev Equilibrium
//
//  Use head version of VPIC.
//
///////////////////////////////////////////////////////////////////////////////
/* #include <H5Part.h> */
#include <math.h>              // This is a standard library
#include <list>                // Sequence container that allows non-contiguous memory allocation
#include <iterator>
#include "vpic/dumpmacros.h"   // In $HOME/vpic_hdf5_rebase/src/vpic
...
// (Groups several related variables into one place)
struct edata {
  species_id sp_id;            /* Species id */
  double       vth;            /* Thermal energy */
  char  fname[256];            /* File to save data */
  // I suppose that fname would have a maximum length of 256 characters?
};
...
#define ELECTRONS_CARRY_CURRENT
...
begin_globals {
  ...
  int nbands;                  // Number of energy bands
  ...
  int nbins;                   // Number of energy bins for energy spectrum diagnostics
  int nx_zone;                 // Number of cells / zone along x for energy spectrum diagnostics
  int ny_zone;                 // Number of cells / zone along y for energy spectrum diagnostics
  int nz_zone;                 // Number of cells / zone along z for energy spectrum diagnostics
...
begin_initialization {
  ...
  // particle tracking
  int particle_tracing = 1;            // 0: no tracing, 1: forward tracing 2: tracing from particle files
  ...
  double vthe    = 0.061237;                      // Electron thermal speed over c

  double wpe_wce = 2.0;                      // Electron plasma freq / electron cyclotron freq
  double bg = 0.000001;                      // Guide field strength
  double theta   = 0.0;                      // B0 = Bx
  double taui    = 500.0/(wpe_wce*mi_me);   // Simulation wci's to run
  ...
  // Problem parameters (for Fadeev equilibrium setup)
  //double beta = 0.1;
  double epsilon = 0.4;
  double kappa = pi/2.0;
  //double alpha = sqrt((1.0/beta)*6.0/5.0);
  ...
  double Lx  = 4.0*pi*L/sqrt(mi_me)*di;    // Size of box in x dimension
  ...
  double topology_y = 1;                     // The prduct must be the number of cores I am using
  double topology_z = 2;
  ...
  double Lpert = L;                       // Wavelength of perturbation
  double dbz = 0.0; //0.05*b0;               // Perturbation in Bz rel. to Bo (Only change here)
  double dbx = 0.0; //-dbz*Lpert/(2.0*Lz);   // Set Bx perturbation so that div(B) = 0
  double dby = 0.0;
  ...
  int nz_zone = 2;            // # of cells / zone along z for energy spectrum diagnostics
...
#define DBX 0.0 //(dbx*cos(2.0*pi*(x-0.5*Lx)/Lpert)*sin(pi*z/Lz))
#define DBY 0.0
#define DBZ 0.0 //(dbz*cos(pi*z/Lz)*sin(2.0*pi*(x-0.5*Lx)/Lpert))

# define DENOM (cosh(z/L) + epsilon * cos(x/L))
#define BX (b0 * sinh(z/L))/DENOM
#define BY b0 * sqrt(1 - epsilon*epsilon)/DENOM
#define BZ (b0 * epsilon * sin(x/L))/DENOM

// In Senbei's deck, the BY component is chosen differently as they have a guide field
// Here below (commented) there are his parameters

//# define DENOM (cosh(z/L) + eps*cos(x/L))
//# define BX b0*sinh(z/L)/DENOM
//# define BY b0*sqrt((1.0-eps*eps)/(DENOM*DENOM) + bg*bg)
//# define BZ b0*eps*sin(x/L)/DENOM

// The current density must be calculated separately

# define JX (b0/L)*sqrt(1.0-epsilon*epsilon)*sinh(z/L)/(DENOM*DENOM)
# define JY (b0/L)*(1.0-epsilon*epsilon)/(DENOM*DENOM)
# define JZ (b0/L)*sqrt(1.0-epsilon*epsilon)*epsilon*sin(x/L)/(DENOM*DENOM)

// This is what appears from Senbei's deck

//# define JY (b0/L)*(1.0-eps*eps)/(DENOM*DENOM)
//# define JX (sinh(z/L)/sqrt((1-eps*eps) + bg*bg*DENOM*DENOM))*JY
//# define JZ (eps*sin(x/L)/sqrt((1-eps*eps) + bg*bg*DENOM*DENOM))*JY

  // Note: everywhere is a region that encompasses the entire simulation
  // In general, regions are specified as logical equations (i.e. x>0 && x+y<2)

  set_region_field( everywhere, 0, 0, 0, BX+DBX, BY+DBY, BZ+DBZ);
...
#ifdef ELECTRONS_CARRY_CURRENT
#define UEDX -JX          // Assumes electrons carry currents
#define UEDY -JY          // The plasma vel is defined by the current density
#define UEDZ -JZ
#define VD sqrt(UEDX*UEDX + UEDY*UEDY + UEDZ*UEDZ)
#define GVD 1./sqrt(1.-VD*VD/(c*c))
#else
#define UEDX -JX/2.0      // Assumes both electrons and ions carry currents
#define UEDY -JY/2.0      // In this case, we have a positron-electron plasma
#define UEDZ -JZ/2.0
#define UIDX -UEDX
#define UIDY -UEDY
#define UIDZ -UEDZ
#define VD sqrt(UEDX*UEDX + UEDY*UEDY + UEDZ*UEDZ)
#define GVD 1./sqrt(1.-VD*VD/(c*c))
// IN CONSTRUCTION: missing the part where I define the module of the velocity for two fluids
#endif
