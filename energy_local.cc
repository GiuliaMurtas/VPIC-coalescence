{
  ...
  if (should_dump(spectrum)) {
    if (energy_diagnostics_first_time) {
      ...
      edParams.push_back(&global->ede_wo_egtb_sironi1);
      edParams.push_back(&global->edH_wo_egtb_sironi1);
      edParams.push_back(&global->ede_wo_egtb_sironi2);
      edParams.push_back(&global->edH_wo_egtb_sironi2);
      edParams.push_back(&global->ede_wo_egtb_sironi3);
      edParams.push_back(&global->edH_wo_egtb_sironi3);
      edParams.push_back(&global->ede_wo_egtb_sironi4);
      edParams.push_back(&global->edH_wo_egtb_sironi4);
    ...
    int ix, iy, iz;
    ix  = int(rank());                /* ix = ix+gpx*( iy+gpy*iz ) */
    iy  = ix/int(global->topology_x);   /* iy = iy+gpy*iz */
    ix -= iy*int(global->topology_x);   /* ix = ix */
    iz  = iy/int(global->topology_y);   /* iz = iz */
    iy -= iz*int(global->topology_y);   /* iy = iy */
